...
This is my take on an R style guide. As such, this is a very long post (long enough to warrant a table of content). Still, I hope it - or at least some parts of it - are useful to you. So without further ado, let's dive into the guide.
...
# If variable and arguments names match, explicit naming can be omitted, but
# be careful about mixing up the argument order, especially when the function
# interface is likely to be re-designed. Unit tests are vital in such cases!
mean <- 1
sd <- 2
std.dev <- sd
rnorm(100, mean, sd)
...
## Syntax
### Assignment
For any assignment, the arrow (<-) is used, not the equal sign (=).
```{r}
x <- 5  # GOOD
x = 5   # BAD
```
...
That's it. You did it and read through the entire post! :)
